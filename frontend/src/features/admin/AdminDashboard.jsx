...
const ImageModal = ({ url, onClose }) => (
  <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.8)', zIndex: 2000, display: 'flex', alignItems: 'center', justifyContent: 'center' }} onClick={onClose}>
...
export default function AdminDashboard({ onClose }) {
  ...
  const handleVerify = async (id, isVerified) => {
    await api.admin.verifyDriver(id, isVerified)
    fetchPending()
  }
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} style={{ padding: 20, maxWidth: 1200, margin: '0 auto' }}>
      ...
      <div style={{ display: 'flex', gap: 10, marginBottom: 20 }}>
        <button onClick={() => setActiveTab('overview')} style={{ padding: '10px 20px', background: 'none', border: 'none', borderBottom: activeTab==='overview'?'2px solid var(--color-primary)':'none' }}>نظرة عامة</button>
        <button onClick={() => setActiveTab('revenue')} style={{ padding: '10px 20px', background: 'none', border: 'none', borderBottom: activeTab==='revenue'?'2px solid var(--color-primary)':'none' }}>الإيرادات</button>
        <button onClick={() => { setActiveTab('pending'); fetchPending() }} style={{ padding: '10px 20px', background: 'none', border: 'none', borderBottom: activeTab==='pending'?'2px solid var(--color-primary)':'none' }}>طلبات السائقين</button>
      </div>
