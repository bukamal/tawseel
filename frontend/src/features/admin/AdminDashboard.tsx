...
export default function AdminDashboard({ onClose }: AdminDashboardProps) {
  ...
  const handleVerify = async (id: string, isVerified: boolean) => {
    await fetch(`${import.meta.env.VITE_API_URL}/api/admin/drivers/${id}/verify`, { method:'PATCH', headers:{'Content-Type':'application/json','X-User-Id':user!.id}, body:JSON.stringify({ is_verified: isVerified }) })
    fetchPending()
  }
  ...
  return (
    <div className="admin-dashboard">
      ...
      <div className="stats-grid">
        <div className="stat-card"><h3>{stats?.total_rides||0}</h3><p>إجمالي الرحلات</p></div>
        <div className="stat-card"><h3>{stats?.completed_rides||0}</h3><p>مكتملة</p></div>
        <div className="stat-card"><h3>{formatPrice(stats?.total_revenue||0)}</h3><p>نقدي</p></div>
        <div className="stat-card"><h3>{formatStarsPrice(stats?.total_stars_revenue||0)}</h3><p>نجوم</p></div>
      </div>
      <div className="tabs">
        <button className={activeTab==='overview'?'active':''} onClick={()=>setActiveTab('overview')}>نظرة عامة</button>
        <button className={activeTab==='revenue'?'active':''} onClick={()=>setActiveTab('revenue')}>الإيرادات</button>
        <button className={activeTab==='pending'?'active':''} onClick={()=>{setActiveTab('pending');fetchPending()}}>طلبات السائقين</button>
      </div>
      {activeTab==='revenue' && (
      ...
      {activeTab==='pending' && (
        <table>
          <thead><tr><th>الاسم</th><th>الهاتف</th><th>المركبة</th><th>إجراء</th></tr></thead>
          <tbody>
            {pending.map((d:any)=> (
              <tr key={d.id}>
                ...
                <td>
                  <button onClick={()=>handleVerify(d.id, true)}>✅ قبول</button>
                  <button onClick={()=>handleVerify(d.id, false)}>❌ رفض</button>
                </td>
              </tr>
            ))}
          </tbody>
