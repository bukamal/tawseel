...
export default function ActiveRide() {
  ...
  const handleCancel = () => showConfirm('إلغاء الرحلة؟', async ok => {
    if (!ok) return
    await fetch(`${import.meta.env.VITE_API_URL}/api/rides/cancel`, { method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({ ride_id: activeRide!.id, cancelled_by: user?.role }) })
    resetRide()
  })
      ...
      <motion.div initial={{y:100}} animate={{y:0}} className="active-ride">
        ...
        {activeRide.driver && (
          <div className="driver-info">
            ...
            <a href={`tel:${activeRide.driver.user_id}`}>📞</a>
          </div>
        )}
        <div className="addresses">
          <div>من: {activeRide.pickup_address}</div>
          <div>إلى: {activeRide.dropoff_address}</div>
        </div>
