...
export default function RideRequest() {
  ...
  useEffect(() => {
    if (pickupLocation && dropoffLocation) estimatePrice()
  ...
  const estimatePrice = async () => {
    ...
    try {
      ...
      setSurge(data.surge_multiplier)
    } catch {
      const vehicle = VEHICLE_TYPES.find(v => v.id === selectedVehicle)!
      const price = vehicle.baseFare + dist * vehicle.perKm
      setEstimatedPrice(Math.round(price))
      setEstimatedStars(convertToStars(price))
    }
  }
  ...
  const handleLocationSelected = (type: 'pickup' | 'dropoff', loc: LocationAddress) => {
  ...
  return (
    <>
      <motion.div initial={{y:100}} animate={{y:0}} className="ride-request">
        <h3>🚗 اطلب توصيلة</h3>
        <div className="location-selector" onClick={()=>setShowPicker('pickup')}>
          <span>📍</span><div><p>نقطة الانطلاق</p><p>{pickupAddress || 'اضغط للتحديد'}</p></div>
        </div>
        ...
        <div className="location-selector" onClick={()=>setShowPicker('dropoff')}>
          <span>🎯</span><div><p>الوجهة</p><p>{dropoffAddress || 'اضغط للتحديد'}</p></div>
        </div>
        <div className="vehicle-types">
          {VEHICLE_TYPES.map(v=> <button key={v.id} className={selectedVehicle===v.id?'active':''} onClick={()=>setSelectedVehicle(v.id)}>{v.icon} {v.name}</button>)}
        </div>
        ...
        {estimatedPrice && distance && (
          <div className="price-estimate">
            <span>المسافة: {distance.toFixed(1)} كم</span>
            {surge>1 && <span className="surge">⚡ ذروة {surge}x</span>}
            <span className="price">{paymentMethod==='stars'?formatStarsPrice(estimatedStars!):formatPrice(estimatedPrice)}</span>
          </div>
        )}
